...
import {SafeAreaView, StyleSheet, View, Text, TextInput, ScrollView, TouchableOpacity} from 'react-native';
import {requestOneTimePayment, requestBillingAgreement, requestDeviceData, PaypalResponse} from 'react-native-paypal';
...
const App = () => {
  ...
  const [billingAgreementDescription, setBillingAgreementDescription] = useState('Billing Description');
      ...
      <ScrollView>
        <View style={styles.wrapper}>
          ...
          <Text style={styles.description}>
            A client token is required for any paypal actions. It should be fetched via server but can be pasted here
            for testing purposes. The button here requires a running server. Current configurations point to this
            project's example server but you'll have to tweak the source code to point to your server/use your sent
            payload
          </Text>
          <TextInput onChangeText={setToken} value={token} style={styles.textInput} placeholder={'Token'} />
          ...
          <Text style={styles.description}>
            This sends the user through the paypal payment flow (purpose of this library). `requestOneTimePayment` will
            request a one time payment while `requestBillingAgreement` will vault a user's payment so they don't have to
            log in each time they want to make a transaction
          </Text>
          <TextInput onChangeText={setAmount} value={amount} style={styles.textInput} />
          ...
          <Text style={styles.sectionTitle}>Step 2b (optional): Fetch device data</Text>
          <Text style={styles.description}>
            Braintree suggests sending your device data in requests for detecting fraudulent actions
          </Text>
          ...
          <Text style={styles.description}>
            For example app this is only hooked up for `requestOneTimePayment`. Send user's payment request to your
            server. You'll need to send the nonce and amount but you may want to also collect other response info.
            Current configurations point to this project's example server but you'll have to tweak the source code to
            point to your server/use your sent payload
          </Text>

          <TouchableOpacity onPress={submitPayment} style={styles.button}>
            <Text style={styles.buttonText}>Complete payment (requires server)</Text>
          </TouchableOpacity>

          {!!error3 && <Text style={styles.errorText}>Error: {error3}</Text>}
          {paymentSuccess && <Text style={styles.description}>🎊🎊 Success! Look in you portal for transaction</Text>}
        </View>
      </ScrollView>
    </SafeAreaView>
  );
